...
class Budget(Base):
    ...
    category: Mapped[str] = mapped_column(String(30))
    amount: Mapped[float]
    month: Mapped[str] = mapped_column(String(7))  # format: 2025-06
    user_id: Mapped[int]
...
def update_page(request: Request, expense_id: int):
    db = SessionLocal()
    expense = db.get(Expense, expense_id)
    db.close()
    return templates.TemplateResponse(
...
def report(request: Request):
    ...
    month_totals = {}
    for expense in expenses:
        month = expense.date[:7]  # extract YYYY-MM from date string
        month_totals[month] = (
            month_totals.get(month, 0) + expense.amount
        )
